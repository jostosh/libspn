...
class GaussianLeaf(VarNode):
    ...
    def _tile_feed(self, feed):
        return tf.tile(tf.expand_dims(feed, -1), [1, 1, self._num_components])

    def _tile_evidence(self, evidence):
        return tf.tile(evidence, [1, self._num_components])

    def _compute_value(self):
        # self._assert_built()
        dist = tfd.Normal(loc=self._mean_variable, scale=tf.sqrt(self._variance_variable))
        evidence_probs = tf.reshape(
            dist.prob(self._tile_feed(self._feed)), (-1, self._compute_out_size()))
    ...
    def _compute_log_value(self):
        ...
        dist = tfd.Normal(loc=self._mean_variable, scale=tf.sqrt(self._variance_variable))
        evidence_probs = tf.reshape(
            dist.log_prob(self._tile_feed(self._feed)), (-1, self._compute_out_size()))
...
class DynamicGaussianLeaf(DynamicVarNode, GaussianLeaf):

    def __init__(self, max_steps, feed=None, num_vars=1, num_components=2, name="GaussianLeaf",
                 data=None, learn_scale=True, total_counts_init=1, learn_dist_params=False,
                 time_major=False):
        self._mean_variable = None
        self._variance_variable = None
        self._num_vars = num_vars
        ...
        if data is not None:
            self.learn_from_data(data, learn_scale=learn_scale)
        super(DynamicVarNode).__init__(
            max_steps=max_steps, feed=feed, name=name, time_major=time_major)
    ...
    def _create_evidence_indicator(self):
        return tf.placeholder_with_default(
            tf.cast(tf.ones_like(self._placeholder), tf.bool),
            shape=[self._max_steps, None, self._num_vars])
    #
    # def _create(self):
    #     super()._create()
    #     self._mean_variable = tf.Variable(
    ...
    def _compute_value(self, time):
        # self._assert_built()
        feed_array = self._unstacked_tensor(self._feed)
        evidence_array = self._unstacked_tensor(self.evidence)
        dist = tfd.Normal(loc=self._mean_variable, scale=tf.sqrt(self._variance_variable))
        evidence_probs = tf.reshape(
            dist.prob(self._tile_feed(feed_array.read(time))), (-1, self._compute_out_size()))
        return tf.where(
            self._tile_evidence(evidence_array.read(time)),
            evidence_probs, tf.ones_like(evidence_probs))

    def _unstacked_tensor(self, tensor):
        return tf.TensorArray(dtype=conf.dtype, size=self._max_steps).unstack(value=tensor)

    def _compute_log_value(self, time):
        # self._assert_built()
        feed_array = self._unstacked_tensor(self._feed)
        evidence_array = self._unstacked_tensor(self.evidence)
        dist = tfd.Normal(loc=self._mean_variable, scale=tf.sqrt(self._variance_variable))
        evidence_probs = tf.reshape(
            dist.log_prob(self._tile_feed(feed_array.read(time))), (-1, self._compute_out_size()))
        return tf.where(
            self._tile_evidence(evidence_array.read(time)),
