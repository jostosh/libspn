...
class OpTestResult:
    ...
    def _to_row(self):
        return [
            ...
            self.input_dist,
            self.setup_time,
            self.weights_init_time,
            self.run_times[0] * 1000,
            np.mean(self.run_times[1:]) * 1000,
            self.test_accuracy,
            self.name
        ]
...
class TestResults:
    ...
    def print(self, file):
        cols = [
            ...
            'test_accuracy',
            'config']
        df = pd.DataFrame(data=[res._to_row() for res in self.cpu_results + self.gpu_results],
                          columns=cols)
        print1(tabulate.tabulate(df, headers='keys', tablefmt='psql', showindex=False), file)
...
class PerformanceTest:
    ...
    def __init__(self, num_decomps, num_subsets, num_mixtures, num_input_mixtures,
                 ...
                 profiles_dir, file, test_op, test_values):
    ...
    def _run_op_test(self, op_fun, input_dist='RAW', multi_nodes=True,
        ...
        # Print
        print2("--> %s: on_gpu=%s, input_dist=%s, inference=%s, multi_nodes=%s, log=%s, name=%s"
               % (op_name, on_gpu, input_dist, ("MPE" if inf_type == spn.InferenceType.MPE
                  else "MARGINAL"), multi_nodes, log, name), self.file)
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            if self.profile:
                ...
                file_name += ("_MPE-LOG" if log else "_MPE") if inf_type == \
                                                                spn.InferenceType.MPE else (
                            "_MARGINAL-LOG" if log else
                            "_MARGINAL")
                file_name += "_%s"

                # Create a profiling report
                profile_report(sess, accumulate_updates, feed, self.profiles_dir,
                               "mnist_training", file_name % ("acc_updt",))
                profile_report(sess, update_spn, None, self.profiles_dir,
                               "mnist_training", file_name % ("spn_updt",))
                profile_report(sess, reset_accumulators, None, self.profiles_dir,
                               "mnist_training", file_name % ("acc_rst",))
        ...
        return OpTestResult(op_name, on_gpu, multi_nodes, spn_size, tf_size,
                            ...
                            run_times, test_accuracy, name)

    def _run_test(self, test_name, op_funs, multi_nodes, inf_type, log, input_dists):
        """Run a single test for multiple ops and devices."""
        cpu_results = []
        gpu_results = []
        for op_fun, mult, val, inp_dist in itertools.product(op_funs, multi_nodes, self.test_values,
                                                             input_dists):
            type_ = type(getattr(spn.conf, self.test_op))
            setattr(spn.conf, self.test_op, type_(val))
            name = '{}={}'.format(self.test_op, type_(val))

            if not self.without_cpu:
                cpu_results.append(
                    self._run_op_test(op_fun, input_dist=inp_dist,
                                      multi_nodes=mult, inf_type=inf_type,
                                      log=bool(log), on_gpu=False, name=name))
            if not self.without_gpu:
                gpu_results.append(
                    self._run_op_test(op_fun, input_dist=inp_dist,
                                      multi_nodes=mult, inf_type=inf_type,
                                      log=bool(log), on_gpu=True, name=name))
        return TestResults(test_name, cpu_results, gpu_results)

    def run(self, log_space, inf_type):
        """Run all tests."""
        print1("Running tests:", self.file)
        results = []

        inf_type = [{'MPE': spn.InferenceType.MPE, 'MARGINAL': spn.InferenceType.MARGINAL}[it]
                    for it in inf_type]

        def name(inf_type, log):
            return 'InferenceType: {}{}'.format(
                'MARGINAL' if inf_type == spn.InferenceType.MARGINAL else 'MPE',
                '-LOG' if log else '')

        for log, inf_t in itertools.product(log_space, inf_type):
            r = self._run_test(name(inf_t, log), [Ops.mnist_01], [True], inf_type=inf_t, log=log,
                               input_dists=["MIXTURE"])
            results.append(r)
...
def main():
    ...
    try:
        t = PerformanceTest(args.num_decomps, args.num_subsets, args.num_mixtures,
                            ...
                            args.profile, args.profiles_dir, f, test_op=args.test_op,
                            test_values=args.test_values)
        t.run(args.log_flags, args.inf_types)
    finally:
        if f is not None:
            f.close()
